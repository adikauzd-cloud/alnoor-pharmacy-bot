...
# ሁሉንም የዋና ሜኑ አዝራሮች የሚይዝ Regex Filter
...
pharmacy_conv = ConversationHandler(
    ...
    states={
        REG_NAME: [MessageHandler(MENU_BUTTONS_REGEX, start), MessageHandler(filters.TEXT & ~filters.COMMAND, reg_get_name)],
        REG_LOCATION: [MessageHandler(MENU_BUTTONS_REGEX, start), MessageHandler(filters.TEXT & ~filters.COMMAND, reg_get_location)],
        REG_PHONE: [MessageHandler(MENU_BUTTONS_REGEX, start), MessageHandler(filters.TEXT & ~filters.COMMAND, reg_get_phone)],
        REG_HOURS: [MessageHandler(MENU_BUTTONS_REGEX, start), MessageHandler(filters.TEXT & ~filters.COMMAND, reg_get_hours)],
        REG_LICENSE: [MessageHandler(MENU_BUTTONS_REGEX, start), MessageHandler(filters.ALL, reg_get_license)],
    },
    fallbacks=[CommandHandler("start", start)]
)
