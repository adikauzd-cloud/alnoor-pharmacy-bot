...
# --- 1. Location Selection Conversation ---
...
# --- 2. Medicine Search Conversation ---
...
search_conv = ConversationHandler(
    ...
    states={
        WAITING_FOR_SEARCH: [
            MessageHandler(filters.Regex(MENU_BUTTONS_REGEX), start),
...
# --- 3. AI Medication Info Conversation ---
...
med_info_conv = ConversationHandler(
    ...
    states={
        WAITING_FOR_MED_INFO: [
            MessageHandler(filters.Regex(MENU_BUTTONS_REGEX), start),
            MessageHandler(filters.ALL & ~filters.COMMAND, analyze_med_info)
        ]
    },
    fallbacks=[CommandHandler("start", start)]
)

# --- 4. Pharmacy Reply & Price Sending ---
...
# --- 5. Pharmacy Registration Conversation ---
...
pharmacy_conv = ConversationHandler(
    ...
    states={
        REG_NAME: [MessageHandler(filters.Regex(MENU_BUTTONS_REGEX), start), MessageHandler(filters.TEXT & ~filters.COMMAND, reg_get_name)],
        REG_LOCATION: [MessageHandler(filters.Regex(MENU_BUTTONS_REGEX), start), MessageHandler(filters.TEXT & ~filters.COMMAND, reg_get_location)],
        REG_PHONE: [MessageHandler(filters.Regex(MENU_BUTTONS_REGEX), start), MessageHandler(filters.TEXT & ~filters.COMMAND, reg_get_phone)],
        REG_HOURS: [MessageHandler(filters.Regex(MENU_BUTTONS_REGEX), start), MessageHandler(filters.TEXT & ~filters.COMMAND, reg_get_hours)],
        REG_LICENSE: [MessageHandler(filters.Regex(MENU_BUTTONS_REGEX), start), MessageHandler(filters.ALL, reg_get_license)],
    },
    fallbacks=[CommandHandler("start", start)]
)
