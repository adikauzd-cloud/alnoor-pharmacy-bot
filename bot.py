...
def add_medicine_to_stock(pharmacy_id, medicine_name, quantity, price=None, expiry_date=None, batch_number=None, reorder_level=5):
    ...
    try:
        # መጀመሪያ መድኃኒቱ መኖሩን ማረጋገጥ
        cursor.execute(f"SELECT id, quantity FROM pharmacy_stock WHERE pharmacy_id = {p} AND LOWER(medicine_name) = LOWER({p})", (pharmacy_id, medicine_name))
        existing = cursor.fetchone()
        
        if existing:
            stock_id, current_qty = existing
            new_qty = current_qty + int(quantity)
            cursor.execute(f"UPDATE pharmacy_stock SET quantity = {p} WHERE id = {p}", (new_qty, stock_id))
        else:
            cursor.execute(f"""
                INSERT INTO pharmacy_stock (pharmacy_id, medicine_name, quantity, price, expiry_date, reorder_level)
                VALUES ({p}, {p}, {p}, {p}, {p}, {p})
            """, (pharmacy_id, medicine_name, quantity, price, expiry_date, reorder_level))
...
def get_pharmacy_stock(pharmacy_id):
    ...
    cursor = conn.cursor()
    p = get_placeholder()
    cursor.execute(f"SELECT id, medicine_name, quantity, price, expiry_date FROM pharmacy_stock WHERE pharmacy_id = {p} ORDER BY medicine_name", (pharmacy_id,))
    rows = cursor.fetchall()
    conn.close()
    return rows
...
def get_low_stock_items(pharmacy_id):
    ...
    cursor = conn.cursor()
    p = get_placeholder()
    cursor.execute(f"SELECT medicine_name, quantity, reorder_level FROM pharmacy_stock WHERE pharmacy_id = {p} AND quantity <= reorder_level", (pharmacy_id,))
    rows = cursor.fetchall()
    conn.close()
    return rows
